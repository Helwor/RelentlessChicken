--------------------------------------------------------------------------------
--------------------------------------------------------------------------------

function gadget:GetInfo()
	return {
		name     = "Chicken Spawner",
		desc     = "Spawns burrows and chickens",
		author   = "quantum, improved by KingRaptor",
		date     = "April 29, 2008", --last update: Mei 7, 2014
		license  = "GNU GPL, v2 or later",
		layer    = 1000001,	-- must do the GameOver() thing only after gadget:awards.lua has finishes detect queen destroyed else queenKill award won't appear.
		enabled  = true --	loaded by default?
	}
end

include("LuaRules/Configs/customcmds.h.lua")

--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
local SAVE_FILE = "Gadgets/unit_spawner.lua"
local speed1,speed2,speed3 = 1,1,false
global1 = 'glob'
global2 = function(x,y) Spring.Echo('global2 func',x) global1=y end
--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
if (gadgetHandler:IsSyncedCode()) then
-- BEGIN SYNCED
-- global1 = "modified in synced"
global2 = function(x) Spring.Echo('modified global2 func',x) end

--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
--
-- Speed-ups and upvalues
--

local Spring		= Spring
local math			= math
local Game			= Game
local table			= table
local ipairs		= ipairs
local pairs			= pairs
local remove 		= table.remove
local random		= math.random
local floor			= math.floor
local round = function (n,dec)
	if dec then 
		local mult = 10^(dec)
		return floor(n*mult + 0.5) / mult
	else
		return floor(n + 0.5)
	end
end
local max			= math.max
local min			= math.min
local ceil			= math.ceil
local UnitDefNames  = UnitDefNames
local UnitDefs      = UnitDefs

local CMD_FIGHT				= CMD.FIGHT
local CMD_ATTACK			= CMD.ATTACK
local CMD_STOP				= CMD.STOP
local CMD_MOVE_STATE		= CMD.MOVE_STATE
local spGiveOrderToUnit		= Spring.GiveOrderToUnit
local spGetTeamUnits		= Spring.GetTeamUnits
local spGetUnitTeam			= Spring.GetUnitTeam
local spGetCommandQueue		= Spring.GetCommandQueue
local spGetGameSeconds		= Spring.GetGameSeconds
local spGetGroundBlocked	= Spring.GetGroundBlocked
local spCreateUnit			= Spring.CreateUnit
local spGetUnitPosition		= Spring.GetUnitPosition
local spGetUnitDefID		= Spring.GetUnitDefID
local spGetUnitSeparation	= Spring.GetUnitSeparation
local spGetGameFrame		= Spring.GetGameFrame
local spSetUnitHealth		= Spring.SetUnitHealth
local spGetUnitsInCylinder	= Spring.GetUnitsInCylinder
local spValidUnitID			= Spring.ValidUnitID
local spGetUnitNearestEnemy = spGetUnitNearestEnemy
local spGetUnitCurrentCommand = Spring.GetUnitCurrentCommand
local spGetUnitVelocity     = Spring.GetUnitVelocity
local spGetUnitMoveTypeData = Spring.GetUnitMoveTypeData
local spGetTeamUnitsCounts = Spring.GetTeamUnitsCounts
local spSetGameRulesParam = Spring.SetGameRulesParam
local spGiveOrderToUnitMap = Spring.GiveOrderToUnitMap
local spGiveOrderToUnitArray = Spring.GiveOrderToUnitArray
local spSetUnitAlwaysVisible = Spring.SetUnitAlwaysVisible
local spEditUnitCmdDesc = Spring.EditUnitCmdDesc
local spFindUnitCmdDesc = Spring.FindUnitCmdDesc
local spTestBuildOrder = Spring.TestBuildOrder
local spGetUnitNearestEnemy = Spring.GetUnitNearestEnemy
local spDestroyUnit = Spring.DestroyUnit
local spGetUnitIsDead = Spring.GetUnitIsDead
local spIsGameOver = Spring.IsGameOver
local spGetModOptions = Spring.GetModOptions
local spCreateFeature = Spring.CreateFeature
local spDestroyFeature = Spring.DestroyFeature
local spGetGroundHeight = Spring.GetGroundHeight
local spGetGameRulesParam = Spring.GetGameRulesParam
local spGetUnitRulesParam = Spring.GetUnitRulesParam
local spSendLuaRulesMsg = Spring.SendLuaRulesMsg
local Echo = Spring.Echo

--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
local unsyncedData = { -- table receiving data from unsynced, as you could imagine
	averageLag = 0.03
	,playersLag = {}
}
local emptyTable	= {}
local roamParam		= {2}
local maxTries		= 500
local maxTriesSmall	= 100
local lava = (Game.waterDamage > 0)
local eggs = tobool(spGetModOptions().eggs)

local pvp = false
local respawnBurrows = false	--the always respawn, not % respawn chance

--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
local luaAI
local chickenTeamID
local computerTeams	= {}
local humanTeams	= {}
local gameFrameOffset = 0

-- anything that needs saving goes in here
local data = {
	queenID = nil,
	miniQueenNum = 1,
	targetCache = nil,
	burrows	= {},
	chickenBirths = {},
	lastWaveTime = 0, 
	timeOfLastSpawn	= 0,	-- when the last burrow was spawned
	waveSchedule = math.huge,	-- wave spawns when this gameframe is reached
	waveNumber	= 0,
	
	eggDecay = {},	-- indexed by featureID, value = game second
	targets = {},	--indexed by unitID, value = teamID
	
	totalTechMod = 0,
	defensePool = 0,
	defenseQuota = 0,
	
	humanAggro = 0,		-- decreases linearly
	humanAggroDelta = 0,	-- resets to zero every wave
	humanAggroPerWave = {},	-- for stats tracking
	
	endgame = false,
	victory = false,
	endMiniQueenNum = 0,
	
	morphFrame = -1,
	morphed = false,
	specialPowerCooldown = 0,
	
	queenTime = 0,
	varFromUnsynced = "not set yet"
}

_G.data = data -- _G can be reached as 'SYNCED' in unsynced
--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
--
-- Load Config
--

do -- load config file
	local CONFIG_FILE = "LuaRules/Configs/spawn_defs.lua"
	local VFSMODE = VFS.RAW_FIRST
	local s = assert(VFS.LoadFile(CONFIG_FILE, VFSMODE))
	local chunk = assert(loadstring(s, file))
	setfenv(chunk, gadget)
	chunk()
end
for k,v in pairs(base_values) do
	if not gadget[k] then
		gadget[k] = v
	end
end

--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
--
-- Utility
--

local function List(set)
	local list,n = {},0
	for k in pairs(set) do
		n=n+1
		list[n] = k
	end
	return list
end
do
	local t,n
	local func = function(ret) n=n+1 t[n]=ret end
	function string:split(sep)
		sep,t,n = sep or ",", {}, 0
		self:gsub('[^'..sep.."]+",func)
		return t
	end
end

local function Count(set)
	local count = 0
	for k in pairs(set) do
		count = count + 1
	end
	return count
end

local noTargetDefID = {}
for name in pairs(noTarget) do
	noTargetDefID[UnitDefNames[name].id]=true
end

local DelayOrder,DistributeOrders,orders
do
	local orders,n_orders = {},0
	local spawns,n_spawns = {},0
	DelayOrder = function(...)
		n_orders = n_orders+1
		orders[n_orders] = {...}
	end
	DelaySpawn = function(t)
		n_spawns = n_spawns+1
		spawns[n_spawns] = t
	end
	-- DistributeOrders = function(percent,minimum)
	DistributeOrders = function(n_to_order)
		if n_orders == 0 then
			return
		end
		-- local n_to_order = min(ceil(n_orders*percent),n_orders)
		n_to_order = min( max(1,n_to_order), n_orders )
		for i=1,n_to_order do
			local order=remove(orders,1)
			if not order then
				Echo('ERROR NO ORDER',n_to_order,n_orders,#orders)
			elseif spValidUnitID(order[1]) then
				spGiveOrderToUnit(unpack(order))
			end
		end
		-- Echo('ordering '..n_to_order..'/'..n_orders)
		n_orders = n_orders - n_to_order
		return true
	end
	DistributeSpawns = function(n_to_spawn)
		if n_spawns == 0 then
			return
		end
		-- local n_to_order = min(ceil(n_orders*percent),n_orders)
		n_to_spawn = min( max(1,n_to_spawn), n_spawns )
		for i=1,n_to_spawn do
			local spawn=remove(spawns,1)
			if not spawn then
				Echo('ERROR NO SPAWN',n_to_spawn,n_spawns,#spawns)
			else
				local eggDecay = spawn.eggDecay
				local id
				if eggDecay or spawn[1]:match('_egg') then
					id = spCreateFeature(unpack(spawn))
					if id and eggDecay then
						data.eggDecay[id] = eggDecay
					end
				else
					local orders,birth
					if spawn.orders then
						orders, birth, spawn.orders = spawn.orders, spawn.birth
					end

					id = spCreateUnit(unpack(spawn))
					if id then
						if birth then
							data.chickenBirths[id] = birth 
						end
						if orders then
							for i,order in ipairs(orders) do
								spGiveOrderToUnit(id,unpack(order))
							end
						end
					end
				end
			end
		end
		-- Echo('ordering '..n_to_order..'/'..n_orders)
		n_spawns = n_spawns - n_to_spawn
		return true
	end
end


local function OrderByTime(t)
    local indexes = {}
    for name,v in pairs(t) do
    	table.insert(indexes,v)
    	v.name = name
    end
    local sort = function(a,b) 
    	return (a.time or 0)<(b.time or 0)
    end
    local newt = table.sort(indexes,sort)
    return indexes
end


--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
--
-- Teams
--

for i, v in ipairs(diff_order) do -- make it bi-directional
	diff_order[v] = i
end


local function CompareDifficulty(...)
	level = 1
	for _, difficulty in ipairs{...} do
		if (diff_order[difficulty] > level) then
			level = diff_order[difficulty]
		end
	end
	return diff_order[level]
end

-- set the mode and difficulty, register teams
if (not gameMode) then -- set human and computer teams
	humanTeams[0]		= true
	computerTeams[1]	= true
	chickenTeamID		= 1
	luaAI			= 0 --defaultDifficulty
else
	local teams = Spring.GetTeamList()
	local lastChickenTeam = nil
	-- the problem is with human controlled chickens, otherwise it counts them as robot-players and difficulty increases very much
	-- probably, ideally this needs to be taught to differentiate between human chickens and human robots...
	for _, teamID in pairs(teams) do
		local teamLuaAI = Spring.GetTeamLuaAI(teamID)
		if teamLuaAI and string.find(string.lower(teamLuaAI), "chicken") then
			lastChickenTeam = teamID
			--break
		end
	end
	local highestLevel = 0
	for _, teamID in pairs(teams) do
		local teamLuaAI = Spring.GetTeamLuaAI(teamID)
		-- check only for chicken AI teams
		if (teamLuaAI and teamLuaAI ~= "" and diff_order[teamLuaAI]) then
			luaAI = teamLuaAI
			highestLevel = CompareDifficulty(teamLuaAI, highestLevel)
			chickenTeamID = teamID
			computerTeams[teamID] = true
		elseif lastChickenTeam and Spring.AreTeamsAllied(teamID,lastChickenTeam) then
			computerTeams[teamID] = true -- count as computer
		else --if not (chickenTeamID and Spring.AreTeamsAllied(teamID, chickenTeamID))
	--then -- well as far as I understood this means to check whether team is chicken enemy then do next
			humanTeams[teamID] = true
				

		end
	end
	if chickenTeamID then
		spSetGameRulesParam("chickenTeamID", chickenTeamID)
	end
	luaAI = highestLevel
end

local gaiaTeamID = Spring.GetGaiaTeamID()
computerTeams[gaiaTeamID] = nil
humanTeams[gaiaTeamID] = nil

local humanTeamsOrdered = {}
for id,_ in ipairs(humanTeams) do humanTeamsOrdered[#humanTeamsOrdered+1] = id end
for i=1, #humanTeamsOrdered do
	if humanTeamsOrdered[i+1] and not Spring.AreTeamsAllied(humanTeamsOrdered[i], humanTeamsOrdered[i+1]) then
		pvp = true
		Spring.Log(gadget:GetInfo().name, LOG.INFO, "Chicken: PvP mode detected")
		break
	end
end

if (luaAI == 0) then
	return false	-- nothing to do here, go home
end

GG.Chicken = {}
GG.varFromSynced = 3
--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
--
-- Difficulty
--


local function SetGlobals(difficulty)
	for key, value in pairs(gadget.difficulties[difficulty]) do
		-- override some base values
		gadget[key] = value
	end
	gadget.difficulties = nil
end

SetGlobals(luaAI or defaultDifficulty) -- set difficulty

-- Echo("chickenTypes, defenders, supporters is ", chickenTypes, defenders, supporters)
local chickenTypesByTime = OrderByTime(chickenTypes)
local defendersByTime = OrderByTime(defenders)
local supportersByTime = OrderByTime(supporters)

-- NOTE Rebalancing rules
-- on the original version, the playerCount impact was too great and made the game impossible when ~5+ players were involved,
-- on the other side, high aggression made the game too easy for experienced player that will get an easy win by farming low level chicken and get a low hp queen in the end
-- Also, some user defined rules were not viable in the original version because some other fixed rule would make a win impossible
-- for example, all the net malus made at each wave would make the game impossible if wave frequency is changed by user without changing those said malus
-- In this consideration, some variables are now time dependant instead of wave dependant, but other variable can alter the impact of the time passed

-- multiplayerFactor = 0.7  -- user rule default value : +0.7 for second player then +0.7^2 for third, +0.7^4 for fourth and so on
-- local multiplayerMalus = (function() for i=2,playerCount do multiplayerMalus = multiplayerMalus +  multiplayerFactor^(i-1) return multiplayerMalus end)()
-- the time reduction for queen appearance and the aggression bonus when killing a burrow should be dynamic and depend on how it is easy to kill a burrow, and 
-- queenHealthMod = queenHealthMod * (0.5*(data.queenTime/baseQueenTime) + 0.5) * ((playerCount/2) + 0.5)




-- adjust for player and chicken bot count
local playerCount = Count(humanTeams)
local malus = playerCount^playerMalus 

easyBurrowKillFactor = ( (((minBaseDistance+maxBaseDistance)/2)/2250)*3 + (burrowSpawnRate/45) ) / 4
spSetGameRulesParam("easyBurrowKillFactor",easyBurrowKillFactor)


-- Aggro rating bonus per burrow killed and tech bonus per burrow killed
for i=1,playerCount do
	burrowRegressTime = burrowRegressTime-((burrowRegressTime/2)/(i*humanAggroTeamScaling))
    humanAggroPerBurrow = humanAggroPerBurrow-((humanAggroPerBurrow/2)/(i*humanAggroTeamScaling))
end
humanAggroPerBurrow = humanAggroPerBurrow * easyBurrowKillFactor-- ratioed by how it is easy to go kill a nest compared to default distance and burrow spawn rate
burrowRegressTime = burrowRegressTime * easyBurrowKillFactor-- ratioed by how it is easy to go kill a nest compared to default distance and burrow spawn rate
spSetGameRulesParam("humanAggroPerBurrow", humanAggroPerBurrow)
spSetGameRulesParam("burrowRegressTime", burrowRegressTime)

techMalusMultiplayer = 0
for i=2,playerCount do
	techMalusMultiplayer = techMalusMultiplayer + (techAccelPerPlayer/(i*techAccelTeamScaling))
end
spSetGameRulesParam("techMalusMultiplayer", techMalusMultiplayer)
humanAggroDefenseFactor = humanAggroDefenseFactor * playerCount
defensePerMin	= defensePerMin*playerCount



burrowSpawnRate = burrowSpawnRate/(malus*0.8 + 0.2)/Count(computerTeams) -- ??? more player == less spawn rate???
spSetGameRulesParam("burrowSpawnRate", burrowSpawnRate)


gracePeriod = max(gracePeriod - gracePenalty*(playerCount - 1), gracePeriodMin)
spSetGameRulesParam("gracePeriod", gracePeriod)

techModByAggro = 0


burrowSpawnTime = burrowSpawnRate*0.20 -- 
spSetGameRulesParam("burrowSpawnTime", burrowSpawnTime)


--humanAggroDecay = humanAggroDecay/playerCount
Echo("Chicken configured for "..playerCount.." players")

local function DisableBuildButtons(unitID, buildNames)
	for _, unitName in ipairs(buildNames) do
		if (UnitDefNames[unitName]) then
			local cmdDescID = spFindUnitCmdDesc(unitID, -UnitDefNames[unitName].id)
			if (cmdDescID) then
				local cmdArray = {disabled = true, tooltip = tooltipMessage}
				spEditUnitCmdDesc(unitID, cmdDescID, cmdArray)
			end
		end
	end
end


--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
--
-- Game Rules
--


local function SetupUnit(unitName)
	spSetGameRulesParam(unitName.."Count", 0)
	spSetGameRulesParam(unitName.."Kills", 0)
end

local baseQueenTime = queenTime
data.queenTime = queenTime


local difficulty = diff_order[luaAI or defaultDifficulty]

--if tobool(spGetModOptions().burrowrespawn) or forceBurrowRespawn then respawnBurrows = true end

local function UpdateUnitCount()
	local teamUnitCounts = spGetTeamUnitsCounts(chickenTeamID)

	--[[ if there are no more chickens of one type, the counter for this type
			 is not updated anymore -> counter keeps last known quantity
			 that's why we force to set all counters to zero first ]]
	for id, type in pairs(chickenTypes) do
		spSetGameRulesParam(id.."Count", 0)
	end
	for id, type in pairs(defenders) do
		spSetGameRulesParam(id.."Count", 0)
	end
	for unitDefID, count in pairs(teamUnitCounts) do
		if (unitDefID ~= "n") then
			spSetGameRulesParam(UnitDefs[unitDefID].name.."Count", count)
		end
	end
end

local function KillOldChicken()
	local now = spGetGameSeconds()
	if data.chickenBirths then
		for unitID, birthDate in pairs(data.chickenBirths) do
			local age = now - birthDate
			if (age > maxAge + random(10)) then
				spDestroyUnit(unitID)
			end
		end
	end
end


--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
--
-- Game End Stuff
--

local function KillAllComputerUnits()
	data.victory = true

	local ggDestroyAlliance = GG.DestroyAlliance
	if not ggDestroyAlliance then
		return
	end

	
	local allyteamsToKill = {}
	local count = 0
	local spGetTeamInfo = Spring.GetTeamInfo
	for teamID in pairs(computerTeams) do
		local _, _, _, _, _, allyTeam = spGetTeamInfo(teamID, false)
		count = count + 1
		allyteamsToKill[count] = allyTeam
	end

	for i = 1, count do
		-- not destroyed directly in the previous loop
		-- because removal breaks the pairs iterator
		ggDestroyAlliance(allyteamsToKill[i])
	end
end

--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
--
-- Spawn Dynamics
--

local function IsPlayerUnitNear(x, z, r)
	for teamID in pairs(humanTeams) do
		if (spGetUnitsInCylinder(x, z, r, teamID)[1]) then
			return true
		end
	end
end

-- used to generate eggs from burrows
local function SpawnEggs(x, y, z)
	local choices,choisesN = {},0
	local now = spGetGameSeconds() + floor(gameFrameOffset/30)

	for name in pairs(chickenTypes) do
		if (not chickenTypes[name].noegg and chickenTypes[name].time <= now) then
			choisesN = choisesN + 1
			choices[choisesN] = name
		end
	end
	for name in pairs(defenders) do
		if (not defenders[name].noegg and defenders[name].time <= now) then
			choisesN = choisesN + 1
			choices[choisesN] = name
		end
	end
	if choisesN <= 0 then return end
	for i=1, burrowEggs do
		local choice = choices[random(choisesN)]
		local rx, rz = random(-30, 30), random(-30, 30)
		DelaySpawn({choice.."_egg", x+rx, y, z+rz, random(-32000, 32000)})
		-- local eggID = spCreateFeature(choice.."_egg", x+rx, y, z+rz, random(-32000, 32000))
		--if (eggID and (not eggs)) then data.eggDecay[eggID] = spGetGameSeconds() + eggDecayTime end
	end
end

--cleans up old eggs
local function DecayEggs()
	if eggs then return end
	local time = spGetGameSeconds()
	for eggID, decayTime in pairs(data.eggDecay) do
		if time >= decayTime then
			spDestroyFeature(eggID)
			data.eggDecay[eggID] = nil
		end
	end
end

--[[
HOW BURROW TARGETING WORKS
In normal mode, burrows will send chickens after a random unit of the owner of the enemy unit closest to the burrow.
In PvP mode, each burrow caches a target ID: that of the nearest enemy structure (but not mines or terraform). This cache is updated every UnitCreated and UnitDestroyed.
	The chickens will attack a random unit belonging to cached target owner.
Idle chickens are automatically set to attack a random target (in GameFrame).
]]--

local function UpdateBurrowTarget(burrowID, targetArg)
	local targetData = data.burrows[burrowID]
	local oldTarget = targetData.targetID
	if not targetArg then
		for id,_ in pairs(data.targets) do
			local testDistance = spGetUnitSeparation(burrowID, id, true) or 100000
			if testDistance < targetData.targetDistance then
				targetData.targetDistance = testDistance
				targetData.targetID = id
			end
		end
	else
		local testDistance = spGetUnitSeparation(burrowID, targetArg, true) or targetData.targetDistance
		if testDistance < targetData.targetDistance then
			targetData.targetDistance = testDistance
			targetData.targetID = targetArg
		end
	end
	--Echo("Final selected target ID: "..data.targetID)
	local validUnitID = spValidUnitID(data.targetID) --in case multiple UnitDestroyed() is called at same frame and burrow happen to choose a target before all Destroyed unit is registered.
	if validUnitID and targetData.targetID ~= oldTarget then
		targetData.targetTeam = spGetUnitTeam(data.targets[data.targetID])
		--DelayOrder(burrowID, CMD_ATTACK, {data.targetID}, 0)
		--Echo("Target for burrow ID ".. burrowID .." updated to target ID " .. data.targetID)
	elseif not validUnitID then
		targetData.targetID = nil
		--DelayOrder(burrowID, CMD_STOP, {}, 0)
		--Echo("Target for burrow ID ".. burrowID .." lost, waiting")
	end
end

local function AttackNearestEnemy(unitID)
	local targetID = spGetUnitNearestEnemy(unitID)
	if (targetID) then
		local tx, ty, tz = spGetUnitPosition(targetID)
		DelayOrder(unitID, CMD_FIGHT, {tx, ty, tz}, 0)
	end
end

local function ChooseTarget(unitID,badTargetID)
	local tries = 0
	local units = {}
	if (not unitID) or unitID==badTargetID or (spGetUnitTeam(unitID) == gaiaTeamID) then
		--makes chicken go for random unit belonging to owner of random player if unitID is NIL
		local humanTeamList = List(humanTeams)
		local n_humans = #humanTeamList
		local humanTeams = {}
		if (not humanTeamList[1]) then
			return
		end
		repeat
			local teamID = humanTeamList[random(n_humans)]

			units = humanTeams[teamID]
			if not units then
				humanTeams[teamID]=spGetTeamUnits(teamID)
				units = humanTeams[teamID]
			end
			tries = tries + 1
		until (units[1] or tries >= 100 )
	else
		--makes chicken go for random unit belonging to owner of closest enemy
		local teamID = spGetUnitTeam(unitID)
		units = spGetTeamUnits(teamID)
	end
	tries = 0
	local targetID
	if (units[2]) then
		local n_units = #units
		repeat
			local r = random(1,n_units)
			targetID = units[r]
			tries = tries + 1
			if not targetID or targetID==badTargetID or (noTargetDefID[spGetUnitDefID(targetID)]) then
				n_units=n_units-1
				remove(units,r)
				targetID=nil
			end
		until (targetID or tries>=100)
	else
		targetID = units[1]
	end
	if not targetID then return end
	return {spGetUnitPosition(targetID)}, targetID
end

local function IsChickenTechAvailable(chickenName, chickenDef, time, currTech)
	if currTech < chickenDef.time then
		return false, true
	end
	if chickenDef.obsolete and (chickenDef.obsolete < currTech) then
		return false
	end
	return true
end

local function ChooseChicken(units, useTech,isChicken)
	local s = spGetGameSeconds() + floor(gameFrameOffset/30)
	local choices,choisesN = {},0
	local techMod = 0
	if useTech then
		techMod = data.totalTechMod
	end
	local currTech = s + techMod
	local min = s * techTimeFloorFactor
	if currTech < min then
		currTech = min
	end
	if currTech > techTimeMax then	-- note max may be lower than min, max takes priority
		currTech = techTimeMax
	end
	if useTech and isChicken then
		spSetGameRulesParam("currTech", currTech)
		spSetGameRulesParam("nextTech", "none")
		spSetGameRulesParam("nextTechTime", currTech)
	end

	local spCandidate, spChickenName
	local n_candidates = 0
	local wantUpper = random(1,4)==4
	local specs = {}
	for _, c in ipairs(units) do
		local chickenName = c.name
		local available, above = IsChickenTechAvailable(chickenName, c, s, currTech)
		if available then
			local chance = floor((c.initialChance or 1) + (s - (c.time/60)) * (c.timeChanceMult or 0))
			for i=1, chance do
				choisesN = choisesN + 1
				choices[choisesN] = chickenName
			end
		end
		if above then
			if not spCandidate then
				spCandidate = c
				specs = {spCandidate}
				n_candidates = 1
				if useTech and isChicken then
					spSetGameRulesParam("nextTech", c.name)
					spSetGameRulesParam("nextTechTime", c.time)
				end
			elseif spCandidate.time<c.time then
				if wantUpper and not gotUpper then
					gotUpper = true
					spCandidate = c
					n_candidates = 1
					specs = {spCandidate}
				else
					break
				end
			elseif spCandidate.time==c.time then
				n_candidates = n_candidates + 1
				specs[n_candidates] = spCandidate
			end

		end
	end

	if (choisesN==0) then
		return
	else
		if n_candidates>0 then
			spChickenName = specs[random(n_candidates)].name
		end
		return choices[random(choisesN)], choices[random(choisesN)], spChickenName
	end
end


local function SpawnChicken(burrowID, spawnNumber, chickenName)
	if spIsGameOver() then return end
	
	local x, z
	local bx, by, bz = spGetUnitPosition(burrowID)
	if (not bx or not by or not bz) then
		return
	end
	local tries		 = 0
	local s			= spawnSquare
	local now		= spGetGameSeconds()
	local burrowTarget	= spGetUnitNearestEnemy(burrowID, 20000, false)
	local tloc = data.targetCache
	if (burrowTarget) then tloc = ChooseTarget(burrowTarget) end
	if pvp and data.burrows[burrowID].targetID then
	local tx, ty, tz = spGetUnitPosition(data.burrows[burrowID].targetID)
	tloc = {tx, ty, tz}
	end

	for i=1, spawnNumber do
		repeat
			x = random(bx - s, bx + s)
			z = random(bz - s, bz + s)
			s = s + spawnSquareIncrement
			tries = tries + 1
		until (not spGetGroundBlocked(x, z) or tries > spawnNumber + maxTriesSmall)
		local spawn = {chickenName, x, by, z, "n", chickenTeamID}
		spawn.orders={{CMD_MOVE_STATE, roamParam, 0}}
		-- local unitID = spCreateUnit(chickenName, x, by, z, "n", chickenTeamID)
		-- if unitID then
			-- DelayOrder(unitID, CMD_MOVE_STATE, roamParam, 0) --// set moveState to roam
			if (tloc) then
				spawn.orders[2] = {CMD_FIGHT, tloc, 0}
				-- DelayOrder(unitID, CMD_FIGHT, tloc, 0)
			end
			spawn.birth = now
			DelaySpawn(spawn)
			-- data.chickenBirths[unitID] = now
		-- end
	end
end

local function SpawnTurret(burrowID, turret, number, force)
	if (not turret) or spGetUnitIsDead(burrowID) then return end
	if data.victory or spIsGameOver() then
		return
	end
	
	local cost = (defenders[turret] and defenders[turret].cost) or 1
	local squadSize = (defenders[turret] and defenders[turret].squadSize) or 1
	squadSize = squadSize*data.defenseQuota/cost * random(75, 125)/100
	
	if ((not force) and random() > squadSize) then
		return
	end
	
	local x, z
	local bx, by, bz = spGetUnitPosition(burrowID)
	if not bx then return end
	local tries = 0
	local s	 = spawnSquare
	local spawnNumber = number or max(squadSize, 1)
	local now = spGetGameSeconds()
	local turretDef

	if not force then
		data.defensePool = data.defensePool - cost*spawnNumber
	end
	
	for i=1, spawnNumber do
		repeat
			x = random(bx - s, bx + s)
			z = random(bz - s, bz + s)
			s = s + spawnSquareIncrement
			tries = tries + 1
		until (not spGetGroundBlocked(x, z) or tries > spawnNumber + maxTriesSmall)
		
		local unitID = spCreateUnit(turret, x, by, z, "n", chickenTeamID) -- FIXME
		turretDef = unitID and UnitDefs[spGetUnitDefID(unitID)]
		if turretDef and turretDef.canMove then
			local burrowTarget = spGetUnitNearestEnemy(burrowID, 20000, false)
			if (burrowTarget) then
				local tloc = ChooseTarget(burrowTarget)
				if tloc then
					DelayOrder(unitID, CMD_FIGHT, tloc, 0)
				end
			end
		--else
		--	Spring.SetUnitBlocking(unitID, false)
		end
	end
end

local function SpawnSupport(burrowID, support, number, force)
	if (not support) or spGetUnitIsDead(burrowID) then return end
	if spIsGameOver() then return end
	local squadSize = (supporters[support] and supporters[support].squadSize) or 1
	squadSize = squadSize * waveSizeMult * random(75, 125)/100
	
	local t = spGetGameSeconds()
	if (t < rampUpTime) then
		squadSize = squadSize * t/rampUpTime
	end
	
	if ((not force) and random() > squadSize)	then
		return
	end
	
	local x, z
	local bx, by, bz = spGetUnitPosition(burrowID)
	if not bx then return end
	local tries = 0
	local s = spawnSquare
	local spawnNumber = number or max(floor(squadSize), 1)
	local burrowTarget	= spGetUnitNearestEnemy(burrowID, 20000, false)
	local tloc = ChooseTarget(burrowTarget)
	local fights,n_fights = {},0
	for i=1, spawnNumber do
		repeat
			x = random(bx - s, bx + s)
			z = random(bz - s, bz + s)
			s = s + spawnSquareIncrement
			tries = tries + 1
		until (not spGetGroundBlocked(x, z) or tries > spawnNumber + maxTriesSmall)
		
		-- local unitID = spCreateUnit(support, x, by, z, "n", chickenTeamID) -- FIXME
		local spawn = {support, x, by, z, "n", chickenTeamID}
		-- if unitID and (burrowTarget) then
		if (burrowTarget) then
			if tloc then
				spawn.orders = {{CMD_FIGHT, tloc, 0}}
				-- DelayOrder(unitID, CMD_FIGHT, tloc, 0)

			end
		end
		DelaySpawn(spawn)
	end
end

local function SpawnBurrow(number, loc, burrowLevel)
	if (data.victory or data.endgame) then return end
	if spIsGameOver() then return end
	
	local t		 = spGetGameSeconds()
	local unitID
	--if t < (gracePeriod/4) then return end
		
	for i=1, (number or 1) do
		local x, y, z
		local tries = 0
		if loc then
			x, y, z = unpack(loc)
		else
			repeat
				x = random(spawnSquare, Game.mapSizeX - spawnSquare)
				z = random(spawnSquare, Game.mapSizeZ - spawnSquare)
				y = spGetGroundHeight(x, z)
				tries = tries + 1
				local badPlace = (lava and spGetGroundHeight(x,z) <= 0) or (spTestBuildOrder(testBuilding, x, y, z, 1) == 2)
				if not badPlace then
						-- human must be in vicinity
						badPlace = true
						local vicinity = spGetUnitsInCylinder(x, z, maxBaseDistance)
						for j=1, #vicinity, 1 do
							if (spGetUnitTeam(vicinity[j]) ~= chickenTeamID) then
								badPlace = false
								break
							end
						end
					if not badPlace then
						-- human must not be in proximity
						local proximity = spGetUnitsInCylinder(x, z, minBaseDistance)
						for j=1, #proximity, 1 do
							if (spGetUnitTeam(proximity[j]) ~= chickenTeamID) then
								badPlace = true
								break
							end
						end
					end
				end
			until (not badPlace or tries > maxTriesSmall)
		end

		-- unitID = spCreateUnit(burrowName, x, y, z, "n", chickenTeamID)
		DelaySpawn({burrowName, x, y, z, "n", chickenTeamID})
		-- handled in UnitCreated()
		--data.burrows[unitID] = {targetID = unitID, targetDistance = 100000}
		--UpdateBurrowTarget(unitID, nil)
		--Spring.SetUnitBlocking(unitID, false)
	end
	return unitID
end
GG.Chicken.SpawnBurrow = SpawnBurrow

-- spawns arbitrary unit(s) obeying min and max distance from human units
-- supports spawning in batches
local function SpawnUnit(unitName, number, minDist, maxDist, target)
	if data.victory then return end
	minDist = minDist or minBaseDistance
	maxDist = maxDist or maxBaseDistance

	local x, y, z
	local tries = 0

	repeat
		if not target then
			x = random(spawnSquare, Game.mapSizeX - spawnSquare)
			z = random(spawnSquare, Game.mapSizeZ - spawnSquare)
		else
			x = random(target[1] - maxDist, target[1] + maxDist)
			z = random(target[3] - maxDist, target[3] + maxDist)
		end
		y = spGetGroundHeight(x, z)
		tries = tries + 1
		if tries > number + maxTries then
			break
		end
		local badPlace = spGetGroundBlocked(x, z)
		if not badPlace then
			-- human must be in vicinity
			badPlace = true
			local vicinity = spGetUnitsInCylinder(x, z, maxDist)
			for i=1, #vicinity, 1 do
				if (spGetUnitTeam(vicinity[i]) ~= chickenTeamID) then
					badPlace = false
					break
				end
			end
			if not badPlace then
				-- human must not be in proximity
				local proximity = spGetUnitsInCylinder(x, z, minDist)
				for i=1, #proximity, 1 do
					if (spGetUnitTeam(proximity[i]) ~= chickenTeamID) then
						badPlace = true
						break
					end
				end
			end
		end
	until not badPlace
	for i=1, (number or 1) do
		-- local unitID = spCreateUnit(unitName, x + random(-spawnSquare, spawnSquare), y, z + random(-spawnSquare, spawnSquare), "n", chickenTeamID)
		local spawn = {unitName, x + random(-spawnSquare, spawnSquare), y, z + random(-spawnSquare, spawnSquare), "n", chickenTeamID}
		spawn.orders = {{CMD_MOVE_STATE, roamParam, 0}}
		-- if unitID then
		-- 	DelayOrder(unitID, CMD_MOVE_STATE, roamParam, 0) --// set moveState to roam
		-- end
	end
end

local function SetMorphFrame()
	data.morphFrame = spGetGameFrame() + random(queenMorphTime[1], queenMorphTime[2])
	--Echo("Morph frame set to: " .. data.morphFrame)
	Echo("Next morph in: " .. ceil((data.morphFrame - spGetGameFrame())/30) .. " seconds")
end

local function SpawnQueen()
	local x, y, z
	local tries = 0
	 
	repeat
		x = random(0, Game.mapSizeX)
		z = random(0, Game.mapSizeZ)
		y = spGetGroundHeight(x, z)
		tries = tries + 1
		local blocking = spTestBuildOrder(testBuildingQ, x, y, z, 1)
		if (blocking == 2) then
			local proximity = spGetUnitsInCylinder(x, z, minBaseDistance)
			for i=1, #proximity, 1 do
				if (spGetUnitTeam(proximity[i]) ~= chickenTeamID) then
					blocking = 1
					break
				end
			end
		end
	until (blocking == 2 or tries > maxTries)
	-- original calculation: the queen health is half impacted by burrows killed and half by playerCount
	queenHealthMod = queenHealthMod * (0.5*(data.queenTime/baseQueenTime) + 0.5) * ((playerCount/2) + 0.5)

	if queenMorphName ~= '' then SetMorphFrame() end
	return spCreateUnit(queenName, x, y, z, "n", chickenTeamID)
end

local function SpawnMiniQueen()
	local x, y, z
	local tries = 0
	 
	repeat
		x = random(0, Game.mapSizeX)
		z = random(0, Game.mapSizeZ)
		y = spGetGroundHeight(x, z)
		tries = tries + 1
		local blocking = spTestBuildOrder(testBuildingQ, x, y, z, 1)
		if (blocking == 2) then
			local proximity = spGetUnitsInCylinder(x, z, minBaseDistance)
			for i=1, #proximity, 1 do
				if (spGetUnitTeam(proximity[i]) ~= chickenTeamID) then
					blocking = 1
					break
				end
			end
		end
	until (blocking == 2 or tries > maxTries)
	local unitID = spCreateUnit(miniQueenName, x, y, z, "n", chickenTeamID)
	
	if unitID then
		local miniQueenTarget	= spGetUnitNearestEnemy(unitID, 20000, false)
		local tloc
		if (miniQueenTarget) then tloc = ChooseTarget(miniQueenTarget) end
		if (tloc) then DelayOrder(unitID, CMD_RAW_MOVE, tloc, 0) end
	end
end


local function ProcessSpecialPowers()
	if data.specialPowerCooldown > 0 then
		return
	end
	local selection
	local time = spGetGameSeconds()
	for i=#specialPowers, 1, -1 do
		--Echo(specialPowers[i].name)
		if (specialPowers[i].time < time) and ((specialPowers[i].obsolete or math.huge) > time) and (specialPowers[i].maxAggro > data.humanAggro) then
			selection = specialPowers[i]
			break
		end
	end
	if not selection then
		return
	end
	--Echo(selection.name .. " selected")
	local count = selection.count or (selection.burrowRatio and (selection.burrowRatio * Count(data.burrows))) or 1
	count = ceil(count)
	
	if selection.tieToBurrow then
		local burrowsOrdered = {}
		local burrowsOrderedCount = 0
		for id in pairs(data.burrows) do
			burrowsOrderedCount = burrowsOrderedCount + 1
			burrowsOrdered[burrowsOrderedCount] = id
		end
		if burrowsOrderedCount > 0 then
			local burrowID = burrowsOrdered[math.random(burrowsOrderedCount)]
			SpawnTurret(burrowID, selection.unit, count, true)
		end
	else
		local target = selection.targetHuman and ChooseTarget()
		SpawnUnit(selection.unit, count, selection.minDist, selection.maxDist, target)
	end
	data.specialPowerCooldown = selection.cooldown or 1*50
	Echo("Chickens unleashing plot: "..selection.name.."!!")
end


local function Wave()
	local t = spGetGameSeconds()
	if data.victory or spIsGameOver() then
		return
	end
	local timeSinceLastWaveMinutes = (t-data.lastWaveTime)/60
	if data.endgame and pvp then
		data.endMiniQueenNum = data.endMiniQueenNum + 1
		if data.endMiniQueenNum == endMiniQueenWaves then
			for i=1,playerCount do SpawnMiniQueen() end
				endMiniQueenNum = 0
			end
	end
	data.specialPowerCooldown = data.specialPowerCooldown - timeSinceLastWaveMinutes*60
	ProcessSpecialPowers()
	
	data.waveNumber = data.waveNumber + 1
	data.humanAggroPerWave[data.waveNumber] = data.humanAggro
	
	local burrowCount = Count(data.burrows)
	--Echo("Wave bonus delta this round: "..data.humanAggroDelta)
	--Echo("Wave bonus this round: "..humanAggro)
	--reduce all chicken appearance times

	if data.humanAggro > 0 then
		techModByAggro = -data.humanAggro * (humanAggroTechTimeRegress * timeSinceLastWaveMinutes)
	else
		techModByAggro = -data.humanAggro * (humanAggroTechTimeProgress * timeSinceLastWaveMinutes)
	end
	data.totalTechMod = data.totalTechMod + techModByAggro + techMalusMultiplayer * timeSinceLastWaveMinutes -- scaled by the ratio of the wave frequency to default frequency
	-- Echo("techModByAggro, techMalusMultiplayer is ",- techModByAggro, techMalusMultiplayer,data.totalTechMod)
	spSetGameRulesParam("totalTechMod", data.totalTechMod)
	spSetGameRulesParam("techModByAggro", techModByAggro)
	--[[
	for chickenName, c in pairs(chickenTypes) do
		c.time = c.time + techModByAggro
		if c.obsolete then c.obsolete = c.obsolete + techModByAggro end
	end
	for chickenName, c in pairs(supporters) do
		c.time = c.time + techModByAggro
		if c.obsolete then c.obsolete = c.obsolete + techModByAggro end
	end
	]]--
	--Echo(burrowCount .. " burrows have reduced tech time by " .. ceil(timeReduction) .. " seconds")
	--Echo("Lifetime tech time reduction: " .. ceil(data.totalTechMod) .. " seconds")
	
	local chicken1Name, chicken2Name, chickenSpName = ChooseChicken(chickenTypesByTime, true, true)
	local totalChickSp = 0
	local turret = ChooseChicken(defendersByTime)
	local support = ChooseChicken(supportersByTime, true)
	local squadNumber = (t*timeSpawnBonus+waveSizeMult) * (baseWaveSize + min(max(data.humanAggro*humanAggroWaveFactor, 0), humanAggroWaveMax)	+ burrowCount*burrowWaveSize) / max(burrowCount, 1)
	if (t < rampUpTime) then
		local oldNumber = squadNumber
		squadNumber = squadNumber * t/rampUpTime
		-- Echo("Ramping up", oldNumber, squadNumber, t, rampUpTime)
	end
	spSetGameRulesParam("squadNumber",squadNumber)
	--if data.queenID then squadNumber = squadNumber/2 end
	local alternateChickName
	local chicken1Number = ceil(waveRatio * squadNumber * chickenTypes[chicken1Name].squadSize)
	local chicken2Number = floor((1-waveRatio) * squadNumber * chickenTypes[chicken2Name].squadSize)


	-- Echo("chicken1Name, chicken2Name, chickenSpName is ", chicken1Name, chicken2Name, chickenSpName)
	local totalChick1, totalChick2 = 0, 0
	if chicken1Name==chicken2Name then
		chicken1Number = chicken1Number+chicken2Number
		chicken2Number = 0
	elseif chicken1Number==1 and chicken2Number==0 then
		alternateChickName = true
	end

	local chickenSpNumber = chickenSpName and ceil(0.01 * squadNumber * chickenTypes[chickenSpName].squadSize) or 0
	if (data.queenID) then
		if alternateChickName then
			chicken1Number, chicken2Number = 0,0
			if (random(waveRatio*100)/100) < waveRatio then
				chicken1Number = 1
			else
				chicken2Number = 1
			end
		end
		if chicken1Number>0 then
			SpawnChicken(data.queenID, chicken1Number*queenSpawnMult, chicken1Name)
			totalChick1 = totalChick1 + chicken1Number
		end
		if chicken2Number>0 then
			SpawnChicken(data.queenID, chicken2Number*queenSpawnMult, chicken2Name)
			totalChick2 = totalChick2 + chicken2Number
		end

		-- if some higher tech exist and option is set, we always spawn a few on the queen spawn, no matter the chance
		if chickenSpNumber>0 and specialChickChance>0 then
			SpawnChicken(data.queenID, chickenSpNumber*queenSpawnMult, chickenSpName)
			totalChickSp = totalChickSp + chickenSpNumber
		end
	end

	-- some code for defenses
	local defensePoolDelta = data.humanAggro * humanAggroDefenseFactor * timeSinceLastWaveMinutes
	if turret then
		defensePoolDelta = defensePoolDelta + defensePerMin	-- don't add defensePerMin for the waves where no turrets can be spawned anyway
	end
	if defensePoolDelta < 0 then defensePoolDelta = 0 end
	spSetGameRulesParam("defensePoolDelta",defensePoolDelta)
	data.defensePool = data.defensePool + defensePoolDelta
	data.defenseQuota = data.defensePool/burrowCount
	
	local spawnDef = false
	local cost = (defenders[turret] and defenders[turret].cost) or 1
	if turret and cost < data.defenseQuota then
		spawnDef = true
		--Echo("Defense pool/quota: " .. data.defensePool .. " / " .. data.defenseQuota)
	end
	
 
	
	for burrowID in pairs(data.burrows) do
		local spawnSpecial = specialChickChance>0 and random(1,round(1/specialChickChance))==1
		
		if alternateChickName then
			chicken1Number, chicken2Number = 0,0
			local roll = random(100)/100
			if roll < waveRatio then
				chicken1Number = 1
			else
				chicken2Number = 1
			end
			-- Echo('rolled '..roll..' vs '..waveRatio, 'chick1:'..chicken1Number, 'chick2:'..chicken2Number)
		end
		if chicken1Number>0 then
			SpawnChicken(burrowID, chicken1Number, chicken1Name)
			totalChick1 = totalChick1 + chicken1Number
		end

		if chicken2Number>0 then
			SpawnChicken(burrowID, chicken2Number, chicken2Name)
			totalChick2 = totalChick2 + chicken2Number
		end

		if spawnSpecial and chickenSpNumber>0 then
			SpawnChicken(burrowID, chickenSpNumber, chickenSpName)
			totalChickSp = totalChickSp + chickenSpNumber
		end
		if spawnDef and not data.endgame then
			SpawnTurret(burrowID, turret)
		end

		SpawnSupport(burrowID, support)
	end
	data.humanAggro = data.humanAggro - timeSinceLastWaveMinutes * humanAggroDecay -- aggro decay per minute of game time
	if data.humanAggro > humanAggroMax then
		data.humanAggro = humanAggroMax
	elseif data.humanAggro < humanAggroMin then
		data.humanAggro = humanAggroMin
	end

	data.humanAggroDelta = 0
	spSetGameRulesParam("humanAggro", data.humanAggro)
	data.lastWaveTime = t
	spSetGameRulesParam("lastWaveTime",t)

	return chicken1Name, chicken2Name, chickenSpName, totalChick1, totalChick2, totalChickSp
end

local function MorphQueen()
	-- store values to be copied
	local tempID = data.queenID
	local x, y, z = spGetUnitPosition(tempID)
	if not (x and y and z) then	-- invalid position somehow, try again in a bit
		data.morphFrame = data.morphFrame + 60
		return
	end
	
	local oldHealth,oldMaxHealth,paralyzeDamage,captureProgress,buildProgress = Spring.GetUnitHealth(tempID)
	local xp = Spring.GetUnitExperience(tempID)
	local heading = Spring.GetUnitHeading(tempID)
	local cmdQueue = spGetCommandQueue(tempID, -1)
	local queenOwner = spGetUnitTeam(tempID)
	
	if Spring.GetUnitIsStunned(tempID) or (spGetUnitRulesParam(tempID, "disarmed") == 1) then	-- postpone morph
		data.morphFrame = data.morphFrame + 60
		return
	end
	
	-- perform switcheroo
	data.queenID = nil
	spDestroyUnit(tempID, false, true, tempID, true)
	if data.morphed == true then
		data.queenID = spCreateUnit(queenName, x, y, z, "n", queenOwner)
	else
		data.queenID = spCreateUnit(queenMorphName, x, y, z, "n", queenOwner)
	end

	if not data.queenID then
		Echo("LUA_ERRRUN chicken queen was not recreated correctly, chicken team unit count / total unit count / maxunits ", Spring.GetTeamUnitCount(queenOwner), #Spring.GetAllUnits(), spGetModOptions().maxunits or 10000)
		return
	end

	data.morphed = not data.morphed
	SetMorphFrame()
	
	-- copy values
	-- position
	Spring.MoveCtrl.Enable(data.queenID)
	--Spring.MoveCtrl.SetPosition(data.queenID, x, y, z)	--needed to reset y-axis position
	--Spring.SpawnCEG("dirt", x, y, z)	--helps mask the transition
	Spring.MoveCtrl.SetHeading(data.queenID, heading)
	Spring.MoveCtrl.Disable(data.queenID)
	local env = Spring.UnitScript.GetScriptEnv(data.queenID)
	Spring.UnitScript.CallAsUnit(data.queenID, env.MorphFunc)
	--health handling
	local _,newMaxHealth				 = Spring.GetUnitHealth(data.queenID)
	newMaxHealth = newMaxHealth * queenHealthMod
	local newHealth = (oldHealth / oldMaxHealth) * newMaxHealth
	-- if newHealth >= 1 then newHealth = 1 end
	Spring.SetUnitMaxHealth(data.queenID, newMaxHealth)
	spSetUnitHealth(data.queenID, {health = newHealth, capture = captureProgress, paralyze = paralyzeDamage, build = buildProgress, })
	-- orders, XP
	Spring.SetUnitExperience(data.queenID, xp)
	if (cmdQueue and cmdQueue[1]) then		--copy order queue
		for i=1,#cmdQueue do
			DelayOrder(data.queenID, cmdQueue[i].id, cmdQueue[i].params, cmdQueue[i].options.coded)
		end
	end
end

--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
--
-- Call-ins
--

function gadget:UnitCreated(unitID, unitDefID, unitTeam)
	local name = UnitDefs[unitDefID].name
	if ( chickenTeamID == unitTeam ) then
		if name == burrowName then
			data.burrows[unitID] = {targetID = unitID, targetDistance = 100000}
			UpdateBurrowTarget(unitID, nil)
		end
		local n = spGetGameRulesParam(name.."Count") or 0
		spSetGameRulesParam(name.."Count", n+1)
	end
	if (alwaysVisible and unitTeam == chickenTeamID) then
		spSetUnitAlwaysVisible(unitID, true)
	end
	if (eggs) then
		DisableBuildButtons(unitID, mexes)
	end
end

function gadget:UnitFinished(unitID, unitDefID, unitTeam)
	--burrow targetting
	local name = UnitDefs[unitDefID].name
	if (humanTeams[unitTeam]) and UnitDefs[unitDefID].isImmobile and (not noTarget.name) then
		--Echo("Building ID "..unitID .." added to target array")
		local x, y, z = spGetUnitPosition(unitID)
		data.targets[unitID] = unitTeam
		--distance check for existing burrows goes here
		for burrow, burrowdata in pairs(data.burrows) do
			UpdateBurrowTarget(burrow, unitID)
		end
	end
end

function gadget:GameStart()
	if pvp then Echo("Chicken: PvP mode initialized") end
	--data.waveSchedule[gracePeriod*30] = true	-- schedule first wave
	data.waveSchedule = gracePeriod * 30 +1
	spSetGameRulesParam('waveSchedule',data.waveSchedule)
end



-- code picked from lag monitor and crude player list
local function CheckPings()
    local gaiaTeamID = Spring.GetGaiaTeamID()
    local teamList = Spring.GetTeamList()
    local players,isHosting,count,totalPing = {},{},0,0
    for i = 1, #teamList do
        local teamID = teamList[i]
        if teamID ~= gaiaTeamID then
            local A, leaderID, isDead, isAiTeam, B, allyTeamID = Spring.GetTeamInfo(teamID, false)
            -- Echo("A",A,"leaderID", leaderID,"isDead", isDead, "isAiTeam",isAiTeam, "B",B, "allyTeamID", allyTeamID)
            local _, _, hostingPlayerID = Spring.GetAIInfo(teamID)
            if hostingPlayerID then
                isHosting[leaderID] = teamID
            end
            if not isDead then
                -- Echo("teamID:"..teamID,"leaderID, isAiTeam, allyTeamID, hostingPlayerID is ", leaderID, isAiTeam, allyTeamID, hostingPlayerID)

                if leaderID < 0 then
                    leaderID = Spring.GetTeamRulesParam(teamID, "initLeaderID") or leaderID
                end
                if leaderID >= 0 then
                    if isAiTeam then
                        leaderID = nil
                    end
                end
                if leaderID then
                    local playerName, active, spectator, teamID, allyTeamID, pingTime, cpuUsage, country, rank = Spring.GetPlayerInfo(leaderID, false)
                    -- Echo("IN GADGET:","playerName",playerName,"active",active, "spectator", spectator,"teamID",teamID,"allyTeamID",allyTeamID
                    -- 	,"pingTime",pingTime,"cpuUsage",cpuUsage,"country",country,"rank",rank)
                    local aiHosted = isHosting[leaderID]
                    players[leaderID] = {name = playerName,active = active,teamID = teamID, pingTime = pingTime, cpuUsage = cpuUsage}
                    count=count+1
                    totalPing = totalPing + pingTime
                    -- Echo("teamID:"..teamID,"LeaderID:",leaderID,"playerName, active, spectator, teamID, allyTeamID, pingTime, cpuUsage, country, rank is ", playerName, active, spectator, teamID, allyTeamID, pingTime, cpuUsage, country, rank)
                end
            end
        end
        -- ~not tested
        -- local isHostedAi = false
        -- local isBackupAi = false
        -- if Spring.GetTeamRulesParam(teamID, "initialIsAiTeam") then
        --     Echo(teamID .. ' is initial Ai Team ')
        --     if Spring.GetTeamLuaAI(teamID) then
        --         -- LuaAIs are always active, unless they exist for the purpose of backup.
        --         if Spring.GetTeamRulesParam(teamID, "backupai") == 1 then
        --             isBackupAi = true
        --         end
        --     else
        --         isHostedAi = true
        --         local _, _, hostingPlayerID = Spring.GetAIInfo(teamID)
        --         -- isAiTeam is false for teams that were AI teams, but had their hosting player drop.
        --         -- AI teams without any hosting player are effectively dead.
        --         if isAiTeam then
        --             -- Echo("leaderID,isAiTeam,hostingPlayerID is ", leaderID,isAiTeam,hostingPlayerID)
        --         end
        --     end
        -- end
    end
    local laggers = 0
    for playerID,player in pairs(players) do
        if player.pingTime>0.8 then
            laggers = laggers + 1
        end
    end
    return round(laggers/count), totalPing/count,totalPing,count -- 50%+ of players are lagging
end



local fps, checkFrequency = 30, 30
local last_t, last_n = spGetGameSeconds(), Spring.GetGameFrame()
local period = 120 -- check average lag every 4 seconds


-- receive from Unsynced
-- use spSendLuaRulesMsg(msg) in unsynced
function gadget:RecvLuaMsg(msg,playerID)
	local chickenMsg = msg:match('^chickenMsg:(.+)')
	if chickenMsg then
		for i,pair in ipairs(chickenMsg:split()) do
		    local sep = pair:find('=')
		    local k,v = pair:sub(1,sep-1),pair:sub(sep+1)
		    if k == 'averageLag' then
		    	unsyncedData.playersLag[playerID] = tonumber(v)
		    	local cnt,total = 0,0
		    	for id,lag in pairs(unsyncedData.playersLag) do
		    		cnt,total = cnt+1, total + lag
		    	end
		    	k = total/cnt
		    end
	    	unsyncedData[k]=tonumber(v) or v
			-- Echo('storing ',k,v,'from unsynced')
		end
	end
end


function gadget:GameFrame(n)
	--if ((n+19 - gracePeriod*30) % (30 * chickenSpawnRate) < 0.1) and (n - gracePeriod*30 > 0) then
	--if data.waveSchedule[n] then
	if n >= data.waveSchedule then
		--data.waveSchedule[n + (30 * chickenSpawnRate)] = true
		data.waveSchedule = n + (30 * chickenSpawnRate) +1
		spSetGameRulesParam('waveSchedule',data.waveSchedule)
		--Echo(data.waveSchedule)
		local args = {Wave()}
		if (args[1]) then
			_G.chickenEventArgs = {type="wave", unpack(args)}
			SendToUnsynced("ChickenEvent")
			_G.chickenEventArgs = nil
		end
	--data.waveSchedule[n] = nil	-- just to be sure
	end

	if ((n+17) % checkFrequency == 0) then -- checkFrequency gets higher if game is slow
		local t = spGetGameSeconds()
		-- this regulate check time, assuming for now game hasn't been paused or speed is not modified by user
		if n - last_n >= period then
			last_n = n
			local lagging,avgPing,totalPing,count = CheckPings()

			spSetGameRulesParam('lagging',lagging)
			if avgPing>0.5 then
				checkFrequency = round((1 + avgPing) * 30)
			else
				checkFrequency = 30
			end
			-- Echo("lagging,avgPing is ", lagging,avgPing,totalPing,count,'Check frequency set to '..checkFrequency)
			
			-- Echo('speed1',speed1,"speed2",speed2,"speed3",speed3)


			-- Echo(" is ",Spring.GetTimer and Sprng.GetTimer(),Spring.DiffTimers)

			-- local period = n - last_n
			-- local time_passed = c - last_c
			-- last_n, last_c = n, c
			-- c = os.clock()
			-- local fps = period / time_passed
		
			-- checkFrequency = max(1,floor(30/(fps/30)))
			-- Echo("checkFrequency,fps,time_passed,'game time' is ","check should be?".. ('%.1f'):format(checkFrequency),('%.1f'):format(fps),('%.1f'):format(time_passed), ('%.1f'):format(t))
			-- spSetGameRulesParam('checkFrequency',checkFrequency)
			-- spSetGameRulesParam('fps',fps)
			-- if fps<10 then
			-- 	spSetGameRulesParam('lagging',1)
			-- 	-- update_messaging = update_messaging + time_passed
			-- 	-- if update_messaging > 20 then
			-- 	-- 	update_messaging = 0
			-- 	-- end

			-- end
		end
		UpdateUnitCount()
		local burrowCount = Count(data.burrows)
		

		local timeSinceLastSpawn = t - data.timeOfLastSpawn
		burrowSpawnTime = burrowSpawnRate*0.20*(burrowCount+1) -- 
		spSetGameRulesParam("burrowSpawnTime",burrowSpawnTime)
		if (burrowSpawnTime < timeSinceLastSpawn and burrowCount < maxBurrows) then
			SpawnBurrow()
			data.timeOfLastSpawn = t
			_G.chickenEventArgs = {type="burrowSpawn"}
			SendToUnsynced("ChickenEvent")
			_G.chickenEventArgs = nil
		end
	
		if (t >= data.queenTime) and (not endlessMode) then
			if (not data.endgame) then
				_G.chickenEventArgs = {type="queen"}
				SendToUnsynced("ChickenEvent")
				_G.chickenEventArgs = nil
				if not pvp then
					local queenID = SpawnQueen()
					if queenID then
						local xp = (malus or 1) - 1
						--Spring.SetUnitExperience(queenID, xp)
						local _, maxHealth = Spring.GetUnitHealth(queenID)
						maxHealth = maxHealth * queenHealthMod
						Spring.SetUnitMaxHealth(queenID, maxHealth)
						spSetUnitHealth(queenID, maxHealth)
						data.queenID = queenID
					end
				else
					--chickenSpawnRate = chickenSpawnRate/2
					for i=1,playerCount do
						SpawnMiniQueen()
					end
				end
				data.endgame = true
			end
		end
		
		if data.miniQueenNum <= #miniQueenTime and (t >= (miniQueenTime[data.miniQueenNum]*data.queenTime)) then
			_G.chickenEventArgs = {type="miniQueen"}
			SendToUnsynced("ChickenEvent")
			_G.chickenEventArgs = nil
			for i=1,playerCount do
				SpawnMiniQueen()
			end
			data.miniQueenNum = data.miniQueenNum + 1
		end
	end
	
	if ((n+29) % (checkFrequency*3)) < 0.1 then
		KillOldChicken()
		DecayEggs()
		local targetID, fixAttempt,fixedTarget
		data.targetCache,targetID = ChooseTarget()
		local targetCache = data.targetCache
		local moves,fights,n_moves,n_fights = {},{},0,0
		local failed = 0
		if (data.targetCache) then
			local chickens = spGetTeamUnits(chickenTeamID)
			for i=1,#chickens do
				local unitID = chickens[i]
				
				local curCmd,_,_,curTarget,_,z,p4,p5 = spGetUnitCurrentCommand(unitID)
				local fail = curCmd==CMD_ATTACK and spGetUnitMoveTypeData(unitID).progressState=='failed'
				if fail then
					failed=failed+1
					-- Echo('Unit '..unitID..' progress failed !')
					if not fixAttempt then
						fixAttempt = true
						local badTarget
						if curTarget then
							if not z then
								badTarget = curTarget
							-- below not working but seems it doesn't matter
							-- elseif round(curTarget)==round(targetCache[1]) and round(z)==round(targetCache[3]) then
							-- 	Echo('BAD TARGET IS THE GROUND TARGET !')
							-- 	badTarget = targetID
							end
						end
						if badTarget then
							local newTargetCache,newTargetID
							newTargetCache,newTargetID = ChooseTarget(nil,badTarget)
							if newTargetCache then
								data.targetCache,targetID = newTargetCache,newTargetID
								fixedTarget = data.targetCache
							end
						end
					end
				end
				if not curCmd or fail then
					--AttackNearestEnemy(unitID)
					if (difficulty > 1) and (unitID == data.queenID) then
						DelayOrder(unitID, CMD_RAW_MOVE, data.targetCache, fail and 0 or CMD.OPT_SHIFT)
					else
						if fail then
							data.targetCache[4],data.targetCache[5]=150,0
							-- n_moves = n_moves+1
							-- moves[n_moves] = unitID
							DelayOrder(unitID, CMD_RAW_MOVE, data.targetCache, 0)
							-- data.targetCache[4],data.targetCache[5] = nil,nil
							-- DelayOrder(unitID, CMD_FIGHT, data.targetCache, CMD.OPT_SHIFT)
						else
							DelayOrder(unitID, CMD_FIGHT, data.targetCache, CMD.OPT_SHIFT)
						end
					end
				end
			end
		end
		-- Echo(failed..' chicken failed their move')
		-- FIXME: don't make chickens lose if they won
		--if (not data.victory) and spIsGameOver() then
		--	KillAllComputerUnits()
		--end
	end
	
	--morphs queen
	if n == data.morphFrame then
		--Echo("Morphing queen")
		MorphQueen()
	end
	-- 
	Echo('spawning '..floor( 1 * (30 / checkFrequency) + 0.5 )..' for this frame')
	DistributeSpawns(floor( 1 * (30 / checkFrequency) + 0.5 )) -- distribute for this frame a maximum of 15 spawns (scaled by checkFrequency), a maximum of 4% of all the orders
	DistributeOrders(floor( 1 * (30 / checkFrequency) + 0.5 )) -- distribute for this frame a maximum of 15 orders (scaled by checkFrequency), a maximum of 4% of all the orders
end


function gadget:UnitDestroyed(unitID, unitDefID, unitTeam)
	data.chickenBirths[unitID] = nil
	if data.targets[unitID] then
		data.targets[unitID] = nil
		for burrow, burrowdata in pairs(data.burrows) do
			if burrowdata.targetID == unitID then		--retarget burrows if needed
				burrowdata.targetID = burrow
				burrowdata.targetDistance = 100000000
				UpdateBurrowTarget(burrow, nil)
			end
		end
	end
	if (unitID == data.queenID) then
		KillAllComputerUnits()
	end
	local name = UnitDefs[unitDefID].name
	if (unitTeam == chickenTeamID) then
		if (chickenTypes[name] or (name == burrowName)) then
			local kills = spGetGameRulesParam(name.."Kills")
			spSetGameRulesParam(name.."Kills", kills + 1)
		end
	end
	if (data.burrows[unitID]) then
		data.burrows[unitID] = nil
		local count = 0
		local burrowsOrdered, n_burrows = {},0
		for _,id in pairs(data.burrows) do
			n_burrows = n_burrows + 1
			burrowsOrdered[n_burrows] = id
			count = count + 1
		end
		
		local aggro = max(data.humanAggro, humanAggroQueenTimeMin)
		aggro = min(aggro, humanAggroQueenTimeMax)
		local reduction = burrowQueenTime*humanAggroQueenTimeFactor*aggro
		reduction = max(reduction, 0)
		spSetGameRulesParam("queenTimeReduction", reduction)
		data.queenTime = max(data.queenTime - reduction, 1)
		
		local oldAggro = data.humanAggro
		data.humanAggro = data.humanAggro + humanAggroPerBurrow
		if data.humanAggro > humanAggroMax then
			data.humanAggro = humanAggroMax
		end
		data.humanAggroDelta = data.humanAggroDelta + data.humanAggro - oldAggro
		spSetGameRulesParam("queenTime", data.queenTime)
		spSetGameRulesParam("humanAggro", data.humanAggro)
		
		data.totalTechMod = data.totalTechMod - burrowRegressTime
		spSetGameRulesParam("totalTechMod", data.totalTechMod)
		
		data.defensePool = data.defensePool + defensePerBurrowKill
		
		-- spawn turrets
		--[[
		if not data.endgame then
			local turret = ChooseChicken(defendersByTime)
			local burrowToDefend = burrowsOrdered[(random(#burrowsOrdered))]
			SpawnTurret(burrowToDefend, turret)
		end
		]]--
	
		if alwaysEggs then SpawnEggs(spGetUnitPosition(unitID)) end
		if (eggs) then SpawnEggs(spGetUnitPosition(unitID)) end
		if (respawnBurrows) or (random() < burrowRespawnChance) then
			--Echo("Respawning burrow")
			SpawnBurrow()
		end
	
		if pvp and data.endgame then
			if count == 0 then KillAllComputerUnits() end
		end
	end
	if (chickenTypes[name] and not chickenTypes[name].noegg) or (defenders[name] and not defenders[name].noegg) or (miniQueenName == name) then
		local x, y, z = spGetUnitPosition(unitID)
		if alwaysEggs then
			-- local eggID = spCreateFeature(name.."_egg", x, y, z, random(-32000, 32000))
			local spawn = {name.."_egg", x, y, z, random(-32000, 32000)}
			if eggDecayTime > 0 and not eggs then
				spawn.eggDecay = spGetGameSeconds() + eggDecayTime
				-- data.eggDecay[eggID] = spGetGameSeconds() + eggDecayTime
			end
			DelaySpawn(spawn)
		end
		if eggs then
			DelaySpawn({name.."_egg", x, y, z, random(-32000, 32000)})
			spCreateFeature(name.."_egg", x, y, z, random(-32000, 32000))
		end
	end
end

--capturing a chicken counts as killing it
function gadget:AllowUnitTransfer(unitID, unitDefID, oldTeam, newTeam, capture)
	if capture then gadget:UnitDestroyed(unitID, unitDefID, oldTeam) end
	return true
end

function gadget:TeamDied(teamID)
	humanTeams[teamID] = nil
	computerTeams[teamID] = nil
end

function gadget:AllowCommand_GetWantedCommand()
	return mexesUnitDefID
end

function gadget:AllowCommand_GetWantedUnitDefID()
	return true
end

function gadget:AllowCommand(unitID, unitDefID, teamID, cmdID, cmdParams, cmdOptions)
	if (eggs) then
		if (mexesUnitDefID[cmdID]) then
			return false -- command was used
		end
	end
	return true	-- command was not used
end

function gadget:FeatureDestroyed(featureID, allyTeam)
	data.eggDecay[featureID] = nil
end

function gadget:GameOver()

	data.morphFrame = -1

	local function ExceedsOne(num)
		num = tonumber(num) or 1
		return num > 1
	end
	local modopts = spGetModOptions()
	local metalmult = tonumber(spGetModOptions().metalmult) or 1
	local energymult = tonumber(spGetModOptions().energymult) or 1
	if ExceedsOne(modopts.metalmult) or ExceedsOne(modopts.metalmult) or (not ExceedsOne((modopts.terracostmult or 1) + 0.001)) then
		Spring.Log(gadget:GetInfo().name, LOG.INFO, "<Chicken> Cheating modoptions, no score sent")
		return
	end
	
	--Echo("<Chicken> AGGRO STATS")
	--for waveNum,aggro in ipairs(data.humanAggroPerWave) do
	--	Echo(waveNum, aggro)
	--end
	
	local time = Spring.GetGameSeconds()
	local score = min(time/data.queenTime, 1) * 1000	-- 1000 points * queen anger %
	if data.endgame then
		score = score + 250	-- +250 points for making it to endgame
	end
	if data.victory then
		score = score + max(60*60 - time, 0) + 250	-- +250 points for winning, +1 point for each second under par
	end
	score = floor(score * scoreMult)	-- multiply by mult
	Spring.SendCommands("wbynum 255 SPRINGIE:score,ID: "..Spring.Utilities.Base64Encode(tostring(spGetGameFrame() + gameFrameOffset).."/"..tostring(floor(score))))
end

function gadget:Load(zip)
	if not (GG.SaveLoad and GG.SaveLoad.ReadFile) then
		Spring.Log(gadget:GetInfo().name, LOG.ERROR, "ERROR: Chicken Spawner failed to access save/load API")
		return
	end
	
	gameFrameOffset = GG.SaveLoad.GetSavedGameFrame()
	
	local saveData = GG.SaveLoad.ReadFile(zip, "Chicken", SAVE_FILE) or {}
	data.queenID = GG.SaveLoad.GetNewUnitID(saveData.queenID)
	data.queenTime = (saveData.queenTime or baseQueenTime) - gameFrameOffset/30
	data.miniQueenNum = saveData.miniQueenNum
	--data.targetCache = saveData.targetCache	-- not needed
	data.burrows = GG.SaveLoad.GetNewUnitIDKeys(saveData.burrows)
	for burrowID, targetData in pairs(data.burrows) do
		targetData.targetID = GG.SaveLoad.GetNewUnitID(targetData.targetID)
	end
	data.chickenBirths = GG.SaveLoad.GetNewUnitIDKeys(saveData.chickenBirths)
	data.timeOfLastSpawn = saveData.timeOfLastSpawn - floor(gameFrameOffset/30)
	data.waveSchedule = saveData.waveSchedule - gameFrameOffset
	data.waveNumber = saveData.waveNumber
	data.eggDecay = GG.SaveLoad.GetNewFeatureIDKeys(saveData.eggDecay)
	data.targets = GG.SaveLoad.GetNewUnitIDKeys(saveData.targets)
	data.totalTechMod = saveData.totalTechMod
	data.defensePool = saveData.defensePool
	data.defenseQuota = saveData.defenseQuota
	
	data.humanAggro = saveData.humanAggro
	data.humanAggroDelta = saveData.humanAggroDelta
	data.humanAggroPerWave = saveData.humanAggroPerWave
	
	data.endgame = saveData.endgame
	data.victory = saveData.victory
	data.endMiniQueenNum = saveData.endMiniQueenNum
	
	data.morphFrame = saveData.morphFrame - gameFrameOffset
	data.morphed = saveData.morphed
	data.specialPowerCooldown = saveData.specialPowerCooldown
	
	spSetGameRulesParam("queenTime", data.queenTime)
	_G.chickenEventArgs = {type="refresh"}
	SendToUnsynced("ChickenEvent")
	_G.chickenEventArgs = nil
end

function gadget:Initialize()
	spSetGameRulesParam("lagging", 0)
	spSetGameRulesParam("queenTime", data.queenTime)
	spSetGameRulesParam("difficulty", difficulty)							-- not dynamic
	spSetGameRulesParam("humanAggroPerBurrow", humanAggroPerBurrow)			-- not dynamic
	spSetGameRulesParam("humanAggro", 0)
	spSetGameRulesParam("gracePeriod", gracePeriod)							-- not dynamic
	spSetGameRulesParam("burrowSpawnTime", burrowSpawnTime)
	spSetGameRulesParam("burrowSpawnRate", burrowSpawnRate)					-- not dynamic
	spSetGameRulesParam("totalTechMod", 0)
	spSetGameRulesParam("techModByAggro", techModByAggro)
	spSetGameRulesParam("techMalusMultiplayer",techMalusMultiplayer)		-- not dynamic
	spSetGameRulesParam("malus", malus)	    								-- not dynamic
	spSetGameRulesParam("burrowRegressTime", burrowRegressTime)	    		-- not dynamic
	spSetGameRulesParam("currTech", 0)
	spSetGameRulesParam("nextTech", chickenTypesByTime[1].name)
	spSetGameRulesParam("nextTechTime", 0)
	spSetGameRulesParam("squadNumber",0)
	spSetGameRulesParam("easyBurrowKillFactor",easyBurrowKillFactor)	    -- not dynamic
	spSetGameRulesParam("queenTimeReduction", burrowQueenTime)

	spSetGameRulesParam("humanAggroDefenseFactor",humanAggroDefenseFactor)	-- not dynamic
	spSetGameRulesParam("defensePoolDelta",0)
	spSetGameRulesParam("defensePerMin",defensePerMin) 						-- not dynamic
	spSetGameRulesParam('waveSchedule',gracePeriod * 30 +1)
	spSetGameRulesParam("lastWaveTime",0)
	spSetGameRulesParam("checkFrequency",30)
	spSetGameRulesParam("fps",30)
	for unitName in pairs(chickenTypes) do
		SetupUnit(unitName)
	end

	for unitName in pairs(defenders) do
		SetupUnit(unitName)
	end

	for unitName in pairs(supporters) do
		SetupUnit(unitName)
	end

	SetupUnit(burrowName)
	SetupUnit(queenName)
end

--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
else
-- END SYNCED
-- BEGIN UNSYNCED
-- note for newb : global are accessible from synced to unsynced, but not modifiable
--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
local Echo = Spring.Echo
local dt, averageLag, dtcount = 0, 0, 0.03
local spGetLastUpdateSeconds = Spring.GetLastUpdateSeconds
local spSendLuaRulesMsg = Spring.SendLuaRulesMsg
local msgKey = 'chickenMsg:'
function gadget:Update()
	dt, dtcount = dt + spGetLastUpdateSeconds(), dtcount+1
	if dt > 15 then
		averageLag = dt / dtcount
		dt, dtcount = 0, 0
		-- send the average lag of the player to synced
		-- local speed1,speed2,paused = Spring.GetGameSpeed()
		spSendLuaRulesMsg(msgKey.."averageLag="..averageLag)
	end
	


	-- Spring.Echo("delta is ", delta,Spring.GetLastUpdateSeconds and Spring.GetLastUpdateSeconds())
	-- if dt>5 then
	-- 	Echo('check!')
	-- end
	speed1,speed2,speed3 = Spring.GetGameSpeed()
end
function WrapToLuaUI() -- 
	if (Script.LuaUI('ChickenEvent')) then
		local chickenEventArgs = {}
		for k, v in spairs(SYNCED.chickenEventArgs) do
			chickenEventArgs[k] = v
		end
		Script.LuaUI.ChickenEvent(chickenEventArgs)
	end
end


function gadget:Initialize()
	gadgetHandler:AddSyncAction('ChickenEvent', WrapToLuaUI)
end

function gadget:Save(zip)
	if not GG.SaveLoad then
		Spring.Log(gadget:GetInfo().name, LOG.ERROR, "ERROR: Chicken Spawner failed to access save/load API")
		return
	end
	
	local chickenTable = SYNCED.data
	chickenTable = Spring.Utilities.MakeRealTable(chickenTable, "Chicken")
	GG.SaveLoad.WriteSaveData(zip, SAVE_FILE, chickenTable)
end
--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
end
-- END UNSYNCED
--------------------------------------------------------------------------------

